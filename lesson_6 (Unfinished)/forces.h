...
struct AdhesionParams {
    int maxSubSitePerNode = 5;        // Number of possible adhesion sites per membrane node

    double siteBindThreshold = 0.1;   // Probability of forming a new adhesion site
    double charUnbindDist = 0.2;      // Characteristic distance for unbinding
    double lambda = 10.0;             // Rate parameter for exponential adhesion length
    double kadh = 1.0;                // Adhesion spring constant

    // Used only by the flat-substrate overload of addAdhesionForces and
    // as the default initial value for detached bindZ entries.
    // For curved substrates, pass a surface function to addAdhesionForces.
    double substrateZ = 0.0;
};
...
inline void initializeAdhesionSites(
    ...
    bindZ.assign(totalSites, adh.substrateZ);
...
// anchor point.
// ------------------------------------------------------------
template <typename RNG, typename SurfaceZFunction>
inline void addAdhesionForces(
    ...
    int expectedSites = numMembraneNodes * adh.maxSubSitePerNode;
    ...
    std::exponential_distribution<double> expDist(adh.lambda);
    ...
    for (int i = 0; i < numMembraneNodes; ++i) {
        ...
        for (int j = 0; j < adh.maxSubSitePerNode; ++j) {
            ...
            if (isAttached[siteIndex]) {
                ...
                double unbindProb = 1.0 - std::exp(-distNodeSite / adh.charUnbindDist);
        ...
        if (bindSiteCount < adh.maxSubSitePerNode) {
            ...
            for (int attempt = 0; attempt < openSites; ++attempt) {
                if (uniform01(rng) < adh.siteBindThreshold) {

                    double randAngle = direction;
                    double randLen = expDist(rng);

                    double x0 = membraneX[i];
                    double y0 = membraneY[i];

                    double nodeSiteX = x0 + randLen * std::cos(randAngle);
                    double nodeSiteY = y0 + randLen * std::sin(randAngle);
                    double nodeSiteZ = surfaceZFunc(nodeSiteX, nodeSiteY);
...
// ------------------------------------------------------------
// Flat-substrate overload
//
// This version keeps the old call style. It places all newly
// created adhesion sites at z = adh.substrateZ.
// ------------------------------------------------------------
...
inline void addAdhesionForces(
    ...
    auto flatSurfaceZ = [&adh](double, double) {
        return adh.substrateZ;
    };
